...
public class Zombie : LivingEntity
{
    ...
    private Renderer zombieRenderer;     // 렌더러 컴포넌트 (몸 색 바꿀 때 사용)

    public float damage = 20f;      // 공격력
    public float timeBetAttack = 0.5f; // 공격 간격
    private float lastAttackTime;   // 마지막 공격 시점

    // 추적할 대상의 존재하는지 알려주는 프로퍼티
    ...
    public void Setup(ZombieData zombieData)
    {
        ...
        health = zombieData.health;  // ★ 수정: health에 health를 넣도록 변경
        ...
        // ★ 추가: ScriptableObject(ZombieData)에 설정된 색으로 외형 색 변경
        if (zombieRenderer != null)
        {
            // 여기에는 하드코딩된 red/black 없음. 전부 zombieData.skinColor 사용.
            zombieRenderer.material.color = zombieData.skinColor;

            // 만약 URP 머티리얼에서 위 코드가 안 먹으면 아래 코드로 시도:
    ...
    private IEnumerator UpdatePath()
    {
        ...
        while (!dead)
        {
            if (hasTarget)
            {
                ...
                if (navMeshAgent != null && navMeshAgent.isOnNavMesh) // ★ 안전 체크
                {
    ...
    public override void Die()
    {
        ...
        if (navMeshAgent != null) // ★ 안전용 null 체크
        {
            navMeshAgent.isStopped = true;
    ...
    {
        // 자신이 사망하지 않았으며,
        // 최신 공격 시점에서 timeBetAttack 이상 시간이 지났다면 공격
